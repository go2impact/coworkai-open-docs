# Cowork Performance Metrics Overview

This system analyzes comprehensive behavioral data from hundreds of thousands of remote workers to generate two core performance metrics. These metrics are calculated using proprietary algorithms that evolve as more data is collected.
...
**Compares your behavioral patterns against:**
...
This metric identifies whether work patterns correlate with successful outcomes, regardless of effort level or time investment. A worker with exceptional efficiency may maintain a solid Go2 Score while working fewer focused hours.

### My Engagement

- **Measures:** Focus intensity during active work periods

**Calculates the density and quality of work-related actions during designated work time, filtering for:**

- Active problem-solving patterns
- Sustained focus indicators
- Context-appropriate tool usage

This metric is role-adjusted. Research roles expect different engagement patterns than transaction-processing roles.
...
- Novel roles with <100 comparable workers show higher variance
...
- System optimizes for detected patterns, which may not capture all value creation methods
...
- Specific behavioral patterns affecting scores
...
Individual scores are visible only to the worker. Management sees aggregated team metrics only. Raw behavioral data is processed algorithmically without human review.
